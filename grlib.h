...
#define GRLIB_PP_ID(v, d, x, i)                                                \
    ((v & 0xff) << 24) | ((d & 0x3ff) << 12) | ((x & 0x1f) << 5) | (i & 0x1f)
#define GRLIB_PP_AHBADDR(a, m, p, c, t)                                        \
    (a & 0xfff00000) | ((m & 0xfff) << 4) | ((p & 1) << 17) | ((c & 1) << 16)  \
            | (t & 0x3)
...
extern int grlib_ahbspp_add(uint32 id,
                            uint32 addr1,
                            uint32 addr2,
                            uint32 addr3,
                            uint32 addr4);
...
extern void grlib_ahbs_add(const struct grlib_ipcore* core,
                           int irq,
                           uint32 addr,
                           uint32 mask);
extern void grlib_ahbm_add(const struct grlib_ipcore* core, int irq);
extern void grlib_apb_add(const struct grlib_ipcore* core,
                          int irq,
                          uint32 addr,
                          uint32 mask);
...
extern void grlib_store_bytes(char* mem, uint32 waddr, uint32* data, int32 sz);
...
extern const struct grlib_ipcore gptimer, irqmp, apbuart0, apbuart1, apbmst, greth, l2c,
        leon3s, srctrl, ns16550, clint, plic, sdctrl, s5test;
