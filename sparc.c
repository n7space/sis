...
static int fpexec(uint32 op3,
                  uint32 rd,
                  uint32 rs1,
                  uint32 rs2,
                  struct pstate* sregs);

static uint32 sub_cc(psr, operand1, operand2, result) uint32 psr;
int32 operand1;
int32 operand2;
int32 result;
{
    ...
    psr = (psr & ~PSR_V)
          | ((((operand1 & ~operand2 & ~result)
             ...
             & PSR_V);
    psr = (psr & ~PSR_C)
          | ((((~operand1 & operand2) | ((~operand1 | operand2) & result))
              >> 11)
             & PSR_C);
    return psr;
}

uint32 add_cc(psr, operand1, operand2, result) uint32 psr;
int32 operand1;
int32 operand2;
int32 result;
{
...
static void log_cc(result, sregs) int32 result;
struct pstate* sregs;
{
...
static int chk_asi(sregs, asi, op3) struct pstate* sregs;
uint32 *asi, op3;
...
static void sparc_set_fsr(fsr) uint32 fsr;
{
    int fround;
...
static int sparc_dispatch_instruction(sregs) struct pstate* sregs;
{
                ...
                switch(op3) {
                    ...
                    case MULScc:
                        ...
                        *rdd = operand1 + operand2;
                        ...
                        sregs->psr =
                                add_cc(sregs->psr, operand1, operand2, *rdd);
                        break;
                    case SMUL: {
            ...
            switch(op3) {
                ...
                case STF:
                    ...
                    mexc = ms->memory_write(
                            address, (uint32*)&sregs->fsi[rd], 2, &ws);
                    sregs->hold += ws;
                    if(mexc) {
                        sregs->trap = TRAP_DEXC;
                    }
...
static int fpexec(op3, rd, rs1, rs2, sregs) uint32 op3, rd, rs1, rs2;
struct pstate* sregs;
{
    ...
    switch(opf) {
        ...
        case FMULs:
            ...
            break;
        case FsMULd:
            if(cputype != CPU_ERC32) { /* FSMULD only supported for LEON3 */
                sregs->fd[rd >> 1] =
...
static int sparc_execute_trap(sregs) struct pstate* sregs;
{
    int32 cwp;
...
static int sparc_check_interrupts(sregs) struct pstate* sregs;
{
...
sparc_disp_regs(struct pstate* sregs, int cwp)
{
    ...
    for(i = 0; i < 8; i++) {
        printf("   %d:  %08X   %08X   %08X   %08X\n",
               ...
               sregs->r[(cwp + i + 16) & 0x7f],
               sregs->r[(cwp + i + 8) & 0x7f],
               sregs->g[i]);
    }
}
...
static void sparc_set_regi(sregs, reg, rval) struct pstate* sregs;
int32 reg;
uint32 rval;
{
    uint32 cwp;
...
static void disp_reg(sregs, reg) struct pstate* sregs;
char* reg;
{
