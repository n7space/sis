...
static void riscv_set_fsr(fsr) uint32 fsr;
{
    int fround;
...
static int set_csr(address, sregs, value) uint32 address;
struct pstate* sregs;
uint32 value;
{
    int res = 0;
...
int get_csr(address, sregs) uint32 address;
struct pstate* sregs;
{
    uint64 tmp;
    switch(address) {
...
static int riscv_dispatch_instruction(sregs) struct pstate* sregs;
{
        ...
        switch(sregs->inst & 3) {
            case 0:
                address = (int32)sregs->r[rs1p]
                          + (int32)EXTRACT_RVC_LW_IMM(sregs->inst);
                switch(funct3) {
            ...
            case 2:
                switch(funct3) {
                    ...
                    case 2: /* LWSP: lw rd, offset[7:2](x2) */
                        address =
                                sregs->r[2] + EXTRACT_RVC_LWSP_IMM(sregs->inst);
                        if(address & 0x3) {
                            sregs->trap = TRAP_LMALI;
                            sregs->wpaddress = address;
                            break;
                    ...
                    case 1: /* FLDSP: ld frd, offset[8:3](x2) */
                        address =
                                sregs->r[2] + EXTRACT_RVC_LDSP_IMM(sregs->inst);
                        if(address & LDDM) {
                            sregs->trap = TRAP_LMALI;
                            sregs->wpaddress = address;
                            break;
                    ...
                    case 4:
                        if((sregs->inst >> 12) & 1) {
                            if(rs1) {
                                if(rs2) { /* add rd, rd, rs2 */
                                    sregs->r[rs1] = (int32)sregs->r[rs1]
                                                    + (int32)sregs->r[rs2];
                                } else { /* jalr x1, rs1, 0 */
#ifdef STAT
                                    sregs->nbranch++;
                    ...
                    case 6: /* SWSP: sw rs2, offset[7:2](x2) */
                        address =
                                sregs->r[2] + EXTRACT_RVC_SWSP_IMM(sregs->inst);
                        if(address & 0x3) {
                            sregs->trap = TRAP_SMALI;
                            sregs->wpaddress = address;
                            break;
                    ...
                    case 5: /* FSDSP: sw frs2, offset[8:3](x2) */
                        address =
                                sregs->r[2] + EXTRACT_RVC_SDSP_IMM(sregs->inst);
                        if(address & LDDM) {
                            sregs->trap = TRAP_SMALI;
                            sregs->wpaddress = address;
                            break;
            ...
            case OP_STORE: /* store instructions */
                ...
                if(ebase.wpwnum) {
                    if((ebase.wphit = check_wpw(sregs, address, funct3 & 3))) {
                        sregs->trap = WPT_TRAP;
                        /* gdb seems to expect that the write goes trough when
                         * the watchpoint is hit, but PC stays on the store
                         * instruction */
                        if(!sis_gdb_break)
                            break;
                    }
...
#ifdef DEBUG
                            if(sis_verbose)
                                printf(" %8" PRIu64 " cpu %d: failed SCQ at "
                                                    "address 0x%08x\n",
                                       sregs->simtime,
                                       sregs->cpu,
                                       address);
                    ...
                    case 0: /* ecall, xret */
                        switch(rs2) {
                            ...
                            case 2: /* xret */
                                ...
                                sregs->mstatus |=
                                        (sregs->mstatus >> 4) & MSTATUS_MIE;
                                sregs->mstatus |=
                                        MSTATUS_MPIE; // set mstatus.mpie
                                rv32_check_lirq(sregs->cpu);
                                if(ebase.coven)
                                    cov_jmp(sregs->pc, npc);
                                break;
                                ...
                                switch(funct3) {
                                    case 0: /* FSGNJ */
                                        sregs->fsi[frd + BEH] =
                                                sregs->fsi[frs1 + BEH];
                                        sregs->fsi[frd + 1 - BEH] =
                                                (sregs->fsi[frs1 + 1 - BEH]
                                                 & 0x7fffffff)
                                                | (sregs->fsi[frs2 + 1 - BEH]
                                                   & 0x80000000);
                                        break;
                                    case 1: /* FSGNJN */
                                        sregs->fsi[frd + BEH] =
...
static int riscv_execute_trap(sregs) struct pstate* sregs;
{
    if(sis_verbose > 1)
...
static int riscv_check_interrupts(sregs) struct pstate* sregs;
{
    if((ext_irl[sregs->cpu])
...
static void riscv_set_regi(sregs, reg, rval) struct pstate* sregs;
int32 reg;
uint32 rval;
{
    if((reg >= 0) && (reg < 32)) {
