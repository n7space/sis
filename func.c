...
static int batch(sregs, fname) struct pstate* sregs;
char* fname;
{
    FILE* fp;
    char* lbuf = NULL;
...
static uint64 limcalc(freq) float32 freq;
{
    uint64 unit, lim;
    double flim;
    char *cmd1, *cmd2;
...
void reset_stat(sregs) struct pstate* sregs;
{
    ebase.tottime = 0.0;
    sregs->pwdtime = 0;
    sregs->ninst = 0;
...
void show_stat(sregs) struct pstate* sregs;
{
...
void init_bpt(sregs) struct pstate* sregs;
{
    int i;
...
static void disp_mem(addr, len) uint32 addr;
uint32 len;
{
    uint32 i;
    union
...
uint32 dis_mem(addr, len) uint32 addr;
uint32 len;
{
    uint32 i, data;
...
void event(cfunc, arg, delta) void (*cfunc)();
int32 arg;
uint64 delta;
{
...
void remove_event(cfunc, arg) void (*cfunc)();
int32 arg;
{
...
void advance_time(endtime) uint64 endtime;
{
    struct evcell* evrem;
    void (*cfunc)();
    uint32 arg;
...
int check_bpt(sregs) struct pstate* sregs;
{
    int32 i;
...
static int run_sim_un(sregs, icount, dis) struct pstate* sregs;
uint64 icount;
int dis;
{
    int irq, mexc, deb;
...
static void run_sim_core(sregs, ntime, deb, dis) struct pstate* sregs;
uint64 ntime;
int deb;
int dis;
{
...
static int run_sim_mp(icount, dis) uint64 icount;
int dis;
{
    uint64 ntime, etime;
    int deb, i, j;
...
int run_sim(icount, dis) uint64 icount;
int dis;
{
    int res;
