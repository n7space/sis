...
create_socket(int port)
{
    ...
    if((new_socket =
                accept(server_fd, (struct sockaddr*)&address, (int*)&addrlen))
       < 0) {
        perror("accept");
        return 0;
    }
    closesocket(server_fd);
    setsockopt(new_socket, SOL_SOCKET, SO_KEEPALIVE, (char*)&opt, sizeof(opt));
    proto = getprotobyname("tcp");
    setsockopt(
            new_socket, proto->p_proto, TCP_NODELAY, (char*)&opt, sizeof(opt));
#ifndef WIN32
...
gdb_remote_exec(char* buf)
{
    ...
    switch(buf[0]) {
        ...
        case 'P': /* write register */
            ...
            while(buf[i] && (buf[i] != '#')) {
                if(cputype == CPU_RISCV) {
                    ...
                    j |= hex(buf[i]);
                    len = (j << 24)
                          | (len >> 8); /* value is in target order! */
                } else
                    len = (len << 4) | hex(buf[i]);
                i++;
            }
        ...
        case 'R': /* restart */
            ...
            break;
        case 'v':
            if(strncmp(&buf[1], "Kill", 4) == 0) { /* restart */
                sim_create_inferior(0, 0, 0, 0);
                strcpy(txbuf, "OK");
            } else if(strncmp(&buf[1], "Run;", 4) == 0) { /* Restart */
                sim_create_inferior(0, 0, 0, 0);
                strcpy(txbuf, "S00");
            } else if(strncmp(&buf[1], "Cont", 4) == 0) { /* continue/step */
                switch(buf[5]) {
                    case '?':
