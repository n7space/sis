...
grlib_init()
{
    ...
    for(i = 0; i < ahbmi; i++)
        if(ahbmcores[i].core->init)
            ahbmcores[i].core->init(0);
    for(i = 0; i < ahbsi; i++)
        if(ahbscores[i].core->init)
            ahbscores[i].core->init(0);
}
...
grlib_reset()
{
    ...
    for(i = 0; i < ahbmi; i++)
        if(ahbmcores[i].core->reset)
            ahbmcores[i].core->reset(0);
    for(i = 0; i < ahbsi; i++)
        if(ahbscores[i].core->reset)
            ahbscores[i].core->reset(0);
}
...
void
grlib_ahbs_add(const struct grlib_ipcore* core,
               int irq,
               uint32 addr,
               uint32 mask)
{
    ...
    for(i = 0; i < ahbsi; i++)
        if((addr >= ahbscores[i].start) && (addr < ahbscores[i].end)) {
            if(ahbscores[i].core->write)
                res = ahbscores[i].core->write(
                        addr & ahbscores[i].mask, data, sz);
            else
                res = 1;
            if(sis_verbose > 2)
...
void
grlib_apb_add(const struct grlib_ipcore* core,
              int irq,
              uint32 addr,
              uint32 mask)
{
...
const struct grlib_ipcore greth = { NULL,
                                    NULL,
                                    grlib_greth_read,
                                    grlib_greth_write,
                                    greth_add };
...
apbmst_add(int irq, uint32 addr, uint32 mask)
{
    grlib_ahbspp_add(GRLIB_PP_ID(VENDOR_GAISLER, GAISLER_APBMST, 0, 0),
                     GRLIB_PP_AHBADDR(addr, mask, 0, 0, 2),
                     0,
                     0,
                     0);
    if(sis_verbose)
...
const struct grlib_ipcore apbmst = { apbmst_init,
                                     apbmst_reset,
                                     apbmst_read,
                                     apbmst_write,
                                     apbmst_add };
...
const struct grlib_ipcore irqmp = { irqmp_init,
                                    irqmp_reset,
                                    irqmp_read,
                                    irqmp_write,
                                    irqmp_add };
...
const struct grlib_ipcore gptimer = { gpt_init,
                                      gpt_reset,
                                      gpt_read,
                                      gpt_write,
                                      gpt_add };
...
#define APBUART0_ADDRESS 0x80000100U
#define APBUART0_IRQ 2

#define APBUART1_ADDRESS 0x80000100U
...
static int ifd1 = -1, ofd1 = -1, ifd2 = -1, ofd2 = -1;
...
sdctrl_add(int irq, uint32 addr, uint32 mask)
{
    grlib_ahbspp_add(GRLIB_PP_ID(VENDOR_GAISLER, GAISLER_SDCTRL, 0, 0),
                     GRLIB_PP_AHBADDR(addr, mask, 1, 1, 2),
                     0,
                     0,
                     0);
    if(sis_verbose)
...
const struct grlib_ipcore sdctrl = { NULL,
                                     NULL,
                                     sdctrl_read,
                                     sdctrl_write,
                                     sdctrl_add };
...
const struct grlib_ipcore srctrl = { NULL,
                                     NULL,
                                     srctrl_read,
                                     srctrl_write,
                                     srctrl_add };
...
const struct grlib_ipcore ns16550 = { NULL,
                                      ns16550_reset,
                                      ns16550_read,
                                      ns16550_write,
                                      ns16550_add };
...
const struct grlib_ipcore clint = { NULL,
                                    NULL,
                                    clint_read,
                                    clint_write,
                                    clint_add };
...
plic_read(uint32 addr, uint32* data)
{
    ...
    if(addr >= PLIC_THRES) {
        ...
        *data = plic_prio[(addr & 0x0ff)
                          >> 2]; // irq priority, not used for now
    }
    if(sis_verbose)
...
plic_write(uint32 addr, uint32* data, uint32 sz)
{
    ...
    if(addr >= PLIC_THRES) {
        ...
        plic_prio[(addr & 0x0ff) >> 2] =
                *data; // irq priority, not used for now
    }
    return 1;
}
...
const struct grlib_ipcore plic = { NULL,
                                   NULL,
                                   plic_read,
                                   plic_write,
                                   plic_add };
...
const struct grlib_ipcore s5test = { NULL,
                                     NULL,
                                     NULL,
                                     s5test_write,
                                     s5test_add };
