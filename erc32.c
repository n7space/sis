...
static void store_bytes(char* mem,
                        uint32 waddr,
                        uint32* data,
                        int sz,
                        int32* ws);
...
static void error_mode(pc) uint32 pc;
{
    mec_ersr |= 0x1;
    decode_ersr();
}
...
static void mec_intack(level) int32 level;
{
    int irq_test;
...
static void mec_irq(level) int32 level;
{
    mec_ipr |= (1 << level);
    chk_irq();
}

static void set_sfsr(fault, addr, asi, read) uint32 fault;
uint32 addr;
uint32 asi;
uint32 read;
{
...
static int32 mec_read(addr, asi, data) uint32 addr;
uint32 asi;
uint32* data;
{
    switch(addr & 0x0ff) {
...
static int mec_write(addr, data) uint32 addr;
uint32 data;
{
    if(sis_verbose > 1)
    ...
    switch(addr & 0x0ff) {
        ...
        case MEC_SSA1: /* 0x20 */
            ...
            if(sis_verbose && mec_wpr[0])
                printf("Segment 1 memory protection enabled (0x02%06x - "
                       "0x02%06x)\n",
                       mec_ssa[0] << 2,
                       mec_sea[0] << 2);
            break;
        ...
        case MEC_SSA2: /* 0x28 */
            ...
            if(sis_verbose && mec_wpr[1])
                printf("Segment 2 memory protection enabled (0x02%06x - "
                       "0x02%06x)\n",
                       mec_ssa[1] << 2,
                       mec_sea[1] << 2);
            break;
...
static uint32 read_uart(addr) uint32 addr;
{
    unsigned tmp;
...
static void write_uart(addr, data) uint32 addr;
uint32 data;
{
    unsigned char c;
...
static void uart_rx(arg) int32 arg;
{
    int32 rsize;
    char rxd;
...
static void uart_intr(arg) int32 arg;
{
...
static void wdog_intr(arg) int32 arg;
{
...
static void rtc_intr(arg) int32 arg;
{
    if(rtc_counter == 0) {
        mec_irq(13);
        if(rtc_cr)
...
static void rtc_scaler_set(val) uint32 val;
{
    rtc_scaler = val & 0x0ff; /* eight-bit scaler only */
}

static void rtc_reload_set(val) uint32 val;
{
    rtc_reload = val;
}

static void gpt_intr(arg) int32 arg;
{
    if(gpt_counter == 0) {
        mec_irq(12);
        if(gpt_cr)
...
static void gpt_scaler_set(val) uint32 val;
{
    gpt_scaler = val & 0x0ffff; /* 16-bit scaler */
}

static void gpt_reload_set(val) uint32 val;
{
    gpt_reload = val;
}

static void timer_ctrl(val) uint32 val;
{
...
memory_write(uint32 addr, uint32* data, int32 sz, int32* ws)
{
    ...
    if((addr >= mem_ramstart) && (addr < (mem_ramstart + mem_ramsz))) {
        if(mem_accprot) {
            ...
            asi = (sregs->psr & 0x080) ? 11 : 10;
            for(i = 0; i < 2; i++)
                wphit[i] = (((asi == 0xa) && (mec_wpr[i] & 1))
                            || ((asi == 0xb) && (mec_wpr[i] & 2)))
                           && ((waddr >= mec_ssa[i])
                               && ((waddr | (sz == 3)) < mec_sea[i]));
...
static char* get_mem_ptr(addr, size) uint32 addr;
uint32 size;
{
...
static int sis_memory_write(addr, data, length) uint32 addr;
...
static int sis_memory_read(addr, data, length) uint32 addr;
char* data;
uint32 length;
{
    char* mem;
